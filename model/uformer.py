...
def make_model(config):  # S: Small, B: Base
    if config["model"]["level"] == "S":  # smaller than T in the Original Uformer paper
        embed_dim = 8
    elif config["model"]["level"] == "B":  # T in the Original Uformer paper
        embed_dim = 16
    print("model loaded!: Uformer-" + config["model"]["level"])
    return Uformer(
        embed_dim=embed_dim,
    )
...
class SepConv2d(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, act_layer=nn.ReLU):
        ...
        self.depthwise = torch.nn.Conv2d(
            in_channels, in_channels, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation, groups=in_channels
        )
...
class ConvProjection(nn.Module):
    def __init__(
        self, dim, heads=8, dim_head=64, kernel_size=3, q_stride=1, k_stride=1, v_stride=1, dropout=0.0, last_stage=False, bias=True
    ):
        super().__init__()
...
class InputProj(nn.Module):
    def __init__(self, in_channel=3, out_channel=64, kernel_size=3, stride=1, norm_layer=None, act_layer=nn.LeakyReLU):
        ...
        self.proj = nn.Sequential(
            nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=stride, padding=kernel_size // 2), act_layer(inplace=True)
        )
...
class Uformer(nn.Module):
    ...
    def forward(self, x, noise, mask=None, benchmark=False):
        # Input Projection
        y = self.input_proj(x)
        y = self.pos_drop(y)
        # Encoder
    ...
    def flops(self, H=512, W=512):
        ...
        # Encoder
        flops += self.encoderlayer_0.flops(H, W) + self.dowsample_0.flops(H, W)
        flops += self.encoderlayer_1.flops(H // 2, W // 2) + self.dowsample_1.flops(H // 2, W // 2)
        flops += self.encoderlayer_2.flops(H // 2**2, W // 2**2) + self.dowsample_2.flops(H // 2**2, W // 2**2)
        flops += self.encoderlayer_3.flops(H // 2**3, W // 2**3) + self.dowsample_3.flops(H // 2**3, W // 2**3)

        # Bottleneck
        flops += self.conv.flops(H // 2**4, W // 2**4)

        # Decoder
        flops += self.upsample_0.flops(H // 2**4, W // 2**4) + self.decoderlayer_0.flops(H // 2**3, W // 2**3)
        flops += self.upsample_1.flops(H // 2**3, W // 2**3) + self.decoderlayer_1.flops(H // 2**2, W // 2**2)
        flops += self.upsample_2.flops(H // 2**2, W // 2**2) + self.decoderlayer_2.flops(H // 2, W // 2)
        flops += self.upsample_3.flops(H // 2, W // 2) + self.decoderlayer_3.flops(H, W)
